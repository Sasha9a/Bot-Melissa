...
export const commands: { command: CommandVkEnum; func: (req: RequestMessageVkModel) => Promise<any>; argv: string }[] = [
  ...
  { command: CommandVkEnum.choose, func: choose, argv: '(действие или действие или ...)' },
  { command: CommandVkEnum.who, func: who, argv: '(вопрос)' },
  { command: CommandVkEnum.horoscope, func: horoscope, argv: '(знак зодиака)' },
...
export const parseMessage = async (message: MessageContext<ContextDefaultState>) => {
  ...
  const members = await vk.api.messages.getConversationMembers({ peer_id: message.peerId, fields: ['bdate', 'sex', 'relation', 'online'] });
