...
export async function checkMessageToMarriage(message: MessageContext<ContextDefaultState>) {
  if (message.replyMessage?.conversationMessageId && ['да', 'нет'].includes(message.text?.toLowerCase())) {
    const marriage: Marriage = await MarriageModule.findOne({ chatId: message.peerId, messageId: message.replyMessage.conversationMessageId });
    if (marriage && (([0, 2].includes(marriage.status) && message.senderId === marriage.userSecondId) || (marriage.status === 1 && message.senderId === marriage.userFirstId))) {
...
export async function processMarriage(info: { userId: number, peerId: number, eventPayload: any }, marriage: Marriage) {
  if (marriage && info.eventPayload?.status === 0 && !marriage.isConfirmed) {
    ...
    await vk.api.messages.send({
      ...
      message: `${await stringifyMention({ userId: info.eventPayload?.userFromId })} увы, но ${await stringifyMention({ userId: info.userId })} отказался(-ась) от предложения вступление в брак`
    }).catch(console.error);
  }
  if (marriage && info.eventPayload?.status === 1 && marriage?.status === 0 && !marriage.isConfirmed) {
    ...
    result = result.concat(`\nСоблюдая торжественный обряд перед регистрацией брака, в присутствии ваших родных и друзей, прошу вас ответить ${await stringifyMention({ userId: info.eventPayload?.userFromId })}`);
    result = result.concat(`, является ли ваше желание стать супругами свободным, взаимным и искренним, готовы ли вы разделить это счастье и эту ответственность, поддерживать и любить друг друга и в горе и в радости?`);
  ...
  if (marriage && info.eventPayload?.status === 3 && marriage?.status === 2 && !marriage.isConfirmed) {
    ...
    result = result.concat(`, которые с давних времен символизируют святость брака, и пусть они напоминают вам, что ваши сердца всегда будут рядом.`);
